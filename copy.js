...
function exploreObject(obj, depth = 0, parentKey = '', accum = {}) {
    const indent = ' '.repeat(depth * 2);
    if (Array.isArray(obj)) {
        obj.forEach((item, index) => {
        ...
        if (typeof obj === 'object' && obj !== null) {
            ...
            for (let key in obj) {
                ...
                if (obj.hasOwnProperty(key)) {
                    ...
                    accum[fullKey] = obj[key];
                    console.log(accum)
                    console.log(`${indent}  ${fullKey}: ${obj[key]}`);

                    //console.log('r: ',obj[key])
                    if (Array.isArray(obj[key])) parentKey += `${key}.`
                    exploreObject(obj[key], depth + 1, parentKey, accum);
                }
            }
        } else {
            //obj aqui es el valor del campo
...
function exploreObject(obj, depth = 0, parentKey = '', accumulator = []) {
    ...
    if (Array.isArray(obj)) {
        obj.forEach((item, index) => {
            console.log(`${indent}  [${index}]`);
            exploreObject(item, depth + 1, `${parentKey}[${index}]`, accumulator);
        });
    }
    else
        if (typeof obj === 'object' && obj !== null) {
            for (let key in obj) {
                if (obj.hasOwnProperty(key)) {
                    const fullKey = `${parentKey}${key}`;
                    console.log(`${indent}  ${fullKey}: ${obj[key]}`);
                    exploreObject(obj[key], depth + 1, `${parentKey}${key}.`, accumulator);
                }
            }
        }
        else {
...
function exploreObject(obj, depth = 0, parentKey = '', accumulator = [], flat = {}) {
    ...
    if (depth === 0 && Object.keys(flat).length > 0) {
